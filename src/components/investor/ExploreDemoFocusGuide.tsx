    ...
    >
      <div className="text-center">
        <h2
          ...
          className="text-[10px] font-black uppercase tracking-[0.35em] text-[var(--cyan)]/75"
        >
      ...
      <div className="grid gap-4 md:grid-cols-2 md:gap-5">
        {areas.map((area) => (
          <article
            key={area.n}
            className="rounded-xl border border-[var(--cyan)]/20 bg-black/45 px-5 py-6 shadow-[0_0_32px_rgba(0, 242, 255,0.06)] backdrop-blur-sm md:px-6 md:py-7"
          >
            <div className="flex items-start gap-3">
              <span className="font-mono text-[11px] font-bold tabular-nums text-[var(--cyan)]/55">{area.n}</span>
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-black uppercase tracking-[0.12em] text-[var(--cyan)] sm:text-[15px]">
                  {area.title}
                </h3>
                ...
                <ul className="mt-4 space-y-2.5 border-t border-white/[0.08] pt-4 text-left text-[12px] leading-relaxed text-white/50 sm:text-[13px]">
                  {area.bullets.map((b) => (
                    <li key={b} className="flex gap-2.5">
                      <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-[var(--cyan)]/70" aria-hidden />
                      <span>{b}</span>
                    </li>
                  ))}
                </ul>
