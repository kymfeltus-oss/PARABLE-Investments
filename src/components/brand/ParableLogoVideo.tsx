...
export function LandingHeroBackgroundVideo({ onEnded }: LandingHeroBackgroundVideoProps = {}) {
  ...
  if (reduceMotion) {
    return <div className="fixed inset-0 z-0 bg-[var(--bg-black)]" aria-hidden />;
  }
    ...
    <div className="fixed inset-0 z-0 max-h-[100dvh] max-w-[100vw] overflow-hidden">
      <div className="absolute inset-0 min-h-0 min-w-0 overflow-hidden">
        <video
          ...
          className="absolute inset-0 box-border max-h-full max-w-full min-h-0 min-w-0 h-full w-full bg-[var(--bg-black)] object-contain object-center md:object-cover"
          src={activeSrc}
          autoPlay
          muted={muted}
          playsInline
      ...
      <div
        className="pointer-events-none absolute bottom-0 right-0 z-[1] h-[min(30vh,10rem)] w-[min(56vw,15rem)] bg-gradient-to-tl from-[var(--bg-black)] from-[10%] via-[var(--bg-black)]/92 to-transparent md:hidden"
        aria-hidden
      />
      <div
